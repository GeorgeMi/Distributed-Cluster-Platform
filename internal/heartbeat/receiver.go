...
func (r *Receiver) handleHeartbeat(data []byte) {
	...
	if !exists {
		...
		if !r.state.AssignNodeToPool(hb.NodeID, newNode) {
			log.Printf("heartbeat receiver: node %s does not match any pool and cannot receive containers", hb.NodeID)
		}
	...
	r.state.UpdateNodeFromHeartbeat(hb)
}
