...
\begin{itemize}
    \item \textbf{Durable audit and full recovery}. Service and container state is already persisted through the Raft log, so a master rebuilds it on restart. The audit log, however, is kept only in memory and is not replicated; persisting and replicating it would be a useful next step.
