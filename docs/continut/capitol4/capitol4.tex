...
\begin{itemize}
    ...
    \item \textbf{TestPlan\_AllOrNothing}. The node has capacity for one replica of 4 CPU but not for two: planning one replica succeeds, planning two replicas fails and no node is assigned.
...
\end{verbatim}
    \item Deploy nginx using the access token from step 1:
\begin{verbatim}
...
\begin{enumerate}
    ...
    \item Query the full audit log:
\begin{verbatim}
curl -H "Authorization: Bearer <TOKEN>" \
  http://localhost:8090/audit
\end{verbatim}
    \item Query with a filter:
\begin{verbatim}
curl -H "Authorization: Bearer <TOKEN>" \
...
\section{Timing and Performance Characteristics}
