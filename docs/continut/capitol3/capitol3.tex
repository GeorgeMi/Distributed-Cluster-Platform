...
\begin{itemize}
    ...
    \item \texttt{internal/cluster} -- in-memory cluster state with thread-safe CRUD operations for all entities, protected by \texttt{sync.RWMutex}
    \item \texttt{internal/heartbeat} -- UDP multicast sender (worker) and receiver (master) with auto-join on first heartbeat
    \item \texttt{internal/fault} -- fault tolerance: periodic health checks every 5 seconds, SUSPECT/DEAD detection, KillContainers for split-brain prevention
    \item \texttt{internal/agent} -- system metrics monitor (CPU, RAM)
\end{itemize}
